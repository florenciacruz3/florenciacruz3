...
**Who this is for:**
Enterprise teams managing 50 or more workspaces and 
planning a move to Microsoft Fabric.
...
- How to audit your tenant manually
- Microsoft Fabric setup
- Advanced Fabric architecture
...
Over years, data environments accumulate problems that stay 
hidden until you try to move. By then, it's too late to fix 
them. 

Three things go wrong in almost every large organization:

1. Teams create workspaces and forget about them — sprawl.
2. People leave, but their data access doesn't get removed 
   — orphaned access, which is a security risk.
3. Licenses keep getting paid for people who no longer use 
   them — waste.

Tenant Scan finds all of this before your migration starts.

Microsoft Fabric is an all-in-one data platform — one 
environment where data is stored, organized, analyzed, and 
turned into reports. At its center is your tenant: your 
organization's private corner of Microsoft's cloud, where 
every workspace, dataset, report, and permission lives.

Without regular governance, that environment accumulates 
problems. They stay invisible until you decide to migrate.

---

## 3. What You Get

Every Tenant Scan delivers three things:

- **Executive Summary with RAG Rating** — a Red, Amber, 
  and Green (RAG) status that gives leadership an immediate 
  read on migration readiness. No technical context needed.

- **Governance Findings** — findings across ten governance 
  failure points, ranked by impact and effort so your team 
  knows what to fix first.

- **Ranked Top 5 Action Plan** — your highest-priority 
  findings turned into a 90-day plan your team can actually 
  execute.

> [!NOTE]
> The intake process runs through a 15-minute questionnaire.
> Where tenant access is required by tier, it's read-only.
> Nothing in your environment is modified.
...
Complete the scan and you'll know exactly where you stand — 
and what to do next.
...
**Every change falls into one of five categories:**

- Make it shorter — cut words that haven't earned their place and cut words that repeat meaning already established.
- "Write like you speak" — advice taken from the Microsoft Writing Style Guide.
- Use contractions — "doesn't," "you'll," "it's" — making the language sound more natural.
- Split long sentences — they're easier to read and scan.
- Speak to the reader directly — use second person ("you," "your").
- Revise weak writing — cut "there is" and "you can" from sentences and start with verbs instead.
